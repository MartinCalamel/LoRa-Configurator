 ...
 * @return 0 if ok
 */
static void set_mode(int handle, LoRa_mode_t mode){
    if (mode){
        up_gpio(handle, M0);
        up_gpio(handle, M1);
    } else {
...
void config_LoRa(int handle, unsigned short address, LoRa_baudrate_t baudrate, LoRa_datarate_t datarate, int freq, LoRa_power_t power){
    ...
    send_msg_uart((unsigned char *)&set_address_cmd, sizeof(set_address_cmd));

    int set_datarate_cmd = set_DataRate(baudrate, datarate);
    send_msg_uart((unsigned char *)&set_datarate_cmd, sizeof(set_datarate_cmd));

    int set_frequency_cmd = set_frequency(freq);
    send_msg_uart((unsigned char *)&set_frequency_cmd, sizeof(set_frequency_cmd));

    int set_power_cmd = set_power(power);
    send_msg_uart((unsigned char *)&set_power_cmd, sizeof(set_power_cmd));
}
 ...
 * @param handle    process for the pin.
...
int send_msg_LoRa(int handle, uint16_t address, uint8_t channel, char *msg, uint8_t msg_size) {
    set_mode(handle, LORA_MODE_TRANSMIT);
